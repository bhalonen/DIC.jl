...
# images=images[1:3]
@polyvar x y t
u_model=monomials((x, y, t), [0, 1,2, 3,4], mono -> (degree(mono,x)<2 && degree(mono,y)<2 && 0<degree(mono,t)<2))#MonomialVector([x*x*t,x*x*t*t,y*y*t,y*y*t*t,x*y*t,x*y*t*t,x*t,y*t,t,1])
v_model=monomials((x, y, t), [0, 1,2, 3,4], mono -> (degree(mono,x)<2 && degree(mono,y)<2 && 0<degree(mono,t)<2))#MonomialVector([x*x*t,x*x*t*t,y*y*t,y*y*t*t,x*y*t,x*y*t*t,x*t,y*t,t,1])
#monomials((x, y, t), [0, 1,2, 3,4])
roi=Rect_ROI(Point(150,50),Point(200,100))
dic_run_params=DIC_Run_Parameters(1,DIC_Setting(700), u_model, v_model)
...
heat_map_exx = make_heat_map(images[1],result.v_transform, time_table[1], result)
heatmap!(heat_map_exx, alpha=.5, clims=extrema(filter(!isnan,heat_map_exx)), colorbar=true,colorbar_title="v")
