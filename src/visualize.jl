...
function make_heat_map(image::Matrix{T}, polynomial::Polynomial, time_frame::Real, result::DIC_Output) where T<:Gray
    ...
    for idxX in x_range, idxY in y_range
        ...
        color_image[new_indexes.x,new_indexes.y]=weighted_color_mean(.5,color_image[new_indexes.x,new_indexes.y],get_color(color_range,min_val,max_val, polynomial(idxX,idxY,time_frame)) )
    end
    return color_image
end