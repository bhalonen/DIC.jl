function find_eulerian(result::DIC_Output{Lagrangian}, time_table::Vector{<:Real})
    samples = map(idx->(time = time_table[idx%length(time_table)+1], point = get_roi_point(result.roi)), 1:500)
