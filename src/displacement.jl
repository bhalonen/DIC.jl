...
function DIC_analysis(dic_input::DIC_Input)
    ...
    roi_samples=map(1:dic_input.dic_run_params.dic_setting.sample_count) do idx
        (frame = Int32(idx%(length(deformed_images))+1),sample_point = get_sample_point(dic_input.roi))
    end
    original_values = map(sample -> reference_image[sample.sample_point].val,roi_samples)
...
function cost_function_builder( deformed_image_itps::Vector{<:AbstractInterpolation},
    ...
    transformed_values = pmap(roi_samples) do sample
        get_transformed_value(deformed_image_itps[sample.frame], sample.sample_point, u, v, time_table[sample.frame+1])
    end
